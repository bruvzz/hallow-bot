...
module.exports = {
  ...
  callback: async (client, interaction) => {
    try {
      ...
      candyData[targetId].candy += amount;
      saveCandyData(candyData);

      const embed = new EmbedBuilder()
        .setColor("Orange")
        .setTitle("🍫 Candy Assigned")
        .setDescription(`Successfully added 🍬 **${amount} candy points** to <@${targetId}>.`)
        .addFields(
          { name: "👤 Target User", value: `<@${targetId}> (\`${targetId}\`)`, inline: true },
          { name: "🍭 Candy Added", value: `\`${amount}\``, inline: true },
      ...
      console.error("Error handling candy-assign command:", error);
      await interaction.editReply({
        content: "An error occurred while assigning candy points.",
        ephemeral: true,
      });
    }
  },
  ...
  name: "candy-assign",
  description: "Manually assign candy points to a user by ID.",
  options: [
    {
      name: "userid",
      description: "The user of the person to give candy to.",
      type: ApplicationCommandOptionType.String,
      required: true,
    },
    {
      name: "amount",
      description: "The amount of candy to assign.",
